    ...
    for (int dataset_index = start_idx; dataset_index < end_idx; dataset_index++) {
        ...
        for (int layer_index = layer_sizes_rows - 2; layer_index >= 0; layer_index--) {
            ...
            if (gpu) {
                ...
                matmul_gpu(context, queue, program, delta_vec, weights_transposed_vec_buf, x_vec, matrix_rows, n_neurons, n_neurons, n_inputs, layer_index);
...
void backward_threading(
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        backward_thread_data[t].weights_transposed_vec_buf = weights_transposed_vec_buf;
